...
export class ParkingPlaceComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.parkingPlaceService
            ...
            .subscribe(
                (res: HttpResponse<IParkingPlace[]>) => this.paginateParkingPlaces(res.body, res.headers),
    ...
    registerChangeInParkingPlaces() {
        this.eventSubscriber = this.eventManager.subscribe('parkingPlaceListModification', response => this.reset());
    }
    ...
    private paginateParkingPlaces(data: IParkingPlace[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.parkingPlaces.push(data[i]);
        }
    }
